...
MLFLOW_EXPERIMENT_NAME = "transaction-classifier"
...
def main():
    ...
    with mlflow.start_run():
        ...
        # --- log parameter eksperimen di awal ---
        mlflow.log_params({
            "epochs_max": EPOCHS,
        ...
        for epoch in range(EPOCHS):
            ...
            # --- di dalam training loop, log metric TIAP epoch ---
            mlflow.log_metrics({"train_loss": total_loss, "val_loss": val_loss}, step=epoch)
        ...
        # --- setelah training selesai, log hasil evaluasi final ---
        model.load_state_dict(torch.load(MODEL_WEIGHTS_PATH, map_location=DEVICE))

        # classification_report bisa diminta dalam bentuk dict, bukan cuma print string
        from sklearn.metrics import classification_report as clf_report

        preds_normal = model(torch.tensor(X_test_normal, dtype=torch.float32).to(DEVICE)).argmax(dim=1).cpu().numpy()
        report_normal = clf_report(y_test_normal, preds_normal, target_names=label_encoder.classes_, output_dict=True, zero_division=0)

        preds_heldout = model(torch.tensor(X_heldout, dtype=torch.float32).to(DEVICE)).argmax(dim=1).cpu().numpy()
        report_heldout = clf_report(y_heldout, preds_heldout, target_names=label_encoder.classes_, output_dict=True, zero_division=0)

        mlflow.log_metrics({
            "accuracy_test_normal": report_normal["accuracy"],
            "accuracy_held_out": report_heldout["accuracy"],  # metric PALING PENTING -- generalisasi asli
            "macro_f1_held_out": report_heldout["macro avg"]["f1-score"],
        })
